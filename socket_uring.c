...
void *zh_malloc(size_t size) {
    ...
    if (!buf) {
        fprintf(stderr, "Fatal error: unable to alloc memory\n");
        exit(1);
    }
    return buf;
}
...
int add_accept_request(int server_socket, struct sockaddr_in *client_addr, socklen_t *client_addr_len) {
    ...
    fprintf(stderr, "JEB:: submitting ACCEPT sqe\n");
...
/*
* Submit an SQE to read from the client socket.
*/
int add_read_request(int client_socket) {
    ...
    // kernel 5.5+ has support for readv(), but not recv() or read().
    // however, my machine's kernel (5.13) and liburing 2.2 does have those, so :shrug:
    io_uring_prep_readv(sqe, client_socket, &req->iov[0], 1, 0);
    io_uring_sqe_set_data(sqe, req);
    fprintf(stderr, "JEB:: submitting READ sqe\n");
    io_uring_submit(&ring);
...
/*
* Submit an SQE that will write data out to a socket.
*/
int add_write_request(struct request *req) {
    ...
    io_uring_sqe_set_data(sqe, req);
    fprintf(stderr, "JEB:: submitting WRITE sqe\n");
    io_uring_submit(&ring);
...
/*
* Utility function to read a file from disk and write to the client socket.
* Uses sendfile() to avoid copying file buffers from kernel to user 
...
void handle_get_method(char *path, int client_socket) {
    ...
    // if path ends with a '/', serve up the index.html
...
/*
* Looks at the HTTP method, and chooses to handle it or not.
* basically, we're only sedrving GETs here, so everything else is told to buzz off.
*/
...
/*
* Utility function to read a line from the incoming buffer.
*/
...
void server_loop(int server_socket) {
    ...
    // add an inital socket accept event into the ring
    add_accept_request(server_socket, &client_addr, &client_addr_len);

    while (1) {
        fprintf(stderr, "blocking for a CQE\n");
        int ret = io_uring_wait_cqe(&ring, &cqe);
        fprintf(stderr, "got a CQE\n");
        ...
        switch (req->event_type) {
            case EVENT_TYPE_ACCEPT:
                // after we've accepted a connection, set up the socket to listen 
                // for more incoming connections.
                add_accept_request(server_socket, &client_addr, &client_addr_len);
